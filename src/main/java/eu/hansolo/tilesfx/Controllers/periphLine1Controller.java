...
public class periphLine1Controller implements Initializable
{
    double sevenGoal;
    double eightGoal;
    double nineGoal;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        vBox.setPrefWidth(messenger.getResolutionizer().setTileWidth(.8));
        vBox.setPrefHeight(messenger.getResolutionizer().screenHeight);

        vBox.getChildren().add(buildTable);
        ...
        table.setMaxWidth(messenger.getResolutionizer().screenWidth);
        table.setMaxHeight(messenger.getResolutionizer().screenHeight);
    ...
    {
        buildTable.setFixedCellSize(messenger.getResolutionizer().screenHeight/13.2);
        ...
        buildTable.setPrefWidth(table.getMaxWidth() * .798);
        buildTable.setPrefHeight(messenger.getResolutionizer().screenHeight);

        hourCol.setPrefWidth(buildTable.getPrefWidth() * .25);
        hourCol.setResizable(false);

        goalCol.setPrefWidth(buildTable.getPrefWidth() * .25);
        goalCol.setResizable(false);

        currentCol.setPrefWidth(buildTable.getPrefWidth() * .25);
        currentCol.setResizable(false);

        differential.setPrefWidth(buildTable.getPrefWidth() * .25);
        differential.setResizable(false);

        double actualGoal = messenger.getMainBuildController().getPeriphBar2Goal() + messenger.getMainBuildController().getPeriphBar1Goal();

        int minuteGoal = (int) actualGoal/450;

        System.out.println(minuteGoal);
    ...
    private void tableRefresh() {
        ...
        {
            if(timeOrbs.get(i).getLine().equals("MIDLAND.ED_A"))
            {
                index = i;
                break;
            }
        ...
        for(int i = 0; i<orbList.size();i++)
        {
            ...
            switch(header)
            {
                ...
                case "5":
                    ...
                    break;
                case "Total":
                    orbList.get(i).setGoal(messenger.getMainBuildController().getPeriphBar2Goal()+messenger.getMainBuildController().getPeriphBar1Goal());
